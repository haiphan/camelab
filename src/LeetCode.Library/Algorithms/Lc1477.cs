...
public class Lc1477Solution {
    public int MinSumOfLengths(int[] arr, int target) {
        int n = arr.Length;
        // left[i] will store the length of the shortest subarray ending at or before index i that sums to target
        int[] left = new int[n]; // 0 means "no valid subarray found yet" (lengths are always >= 1)
        int sum = 0, best = int.MaxValue;
        for (int i = 0, j = 0; i < n; i++) {
            // sum of the current window [j..i]
            sum += arr[i];
            while (sum > target)
            {
                sum -= arr[j++];
            ...
            if (sum == target) {
                ...
                if (best == 2)
                {
                    break; // 2 is the theoretical minimum (two length-1 subarrays)
                }
            }
            if (i > 0) {
                if (left[i] == 0)
